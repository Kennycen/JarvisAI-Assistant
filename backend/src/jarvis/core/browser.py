...
# Playwright can raise its Chromium tab, but not the OS window that contains it.
_MACOS_ACTIVATE = (
    'tell application "System Events" to set frontmost of the first process '
    'whose name is "{name}" to true'
)

_ALLOWED_KEYS = frozenset(
    {"Enter", "Escape", "Tab", "ArrowDown", "ArrowLeft", "ArrowRight", "ArrowUp", "Backspace"}
)


async def _bring_window_to_front(page: Page, process_name: str | None) -> None:
    """Best effort: never let a failed window raise stop the browser being used."""
    with contextlib.suppress(Exception):
        await page.bring_to_front()

    if sys.platform != "darwin" or not process_name:
        return

    with contextlib.suppress(Exception):
        await asyncio.to_thread(
            subprocess.run,
            ["osascript", "-e", _MACOS_ACTIVATE.format(name=process_name)],
            check=False,
            capture_output=True,
            timeout=2,
        )
...
class BrowserManager:
    """Own one isolated browser that Jarvis drives on the user's behalf."""
    ...
    async def start(self) -> None:
        ...
        if not self._headless:
            await _bring_window_to_front(self._page, Path(chromium.executable_path).name)

    async def close(self) -> None:
        async with self._lock:
            # Each teardown step is independent: a failure in one must not strand the rest,
            # because this runs from a session shutdown callback.
    ...
    async def read_page(self, *, max_chars: int = 12_000) -> dict[str, str | bool]:
        ...
        async with self._lock:
            try:
                ...
                raise BrowserError("The page content was not available in time.") from exc
    ...
    async def take_screenshot(self, destination: Path) -> dict[str, str]:
        page = await self._get_page()

        async with self._lock:
            try:
                destination.parent.mkdir(parents=True, exist_ok=True)
                await page.screenshot(type="png", path=str(destination))
                return {"path": str(destination), "url": page.url, "title": await page.title()}
            except Exception as exc:
    ...
    async def _resolve_target(self, page: Page, target: str) -> Locator:
        ...
        for exact in (True, False):
            locator = page.get_by_text(target, exact=exact)
            if await locator.count():
                return locator.first
    ...
    async def _resolve_textbox(self, page: Page, target: str) -> Locator:
        ...
        # "Search" style fields are often unlabelled, so fall back to shape matching.
        normalized_target = target.casefold()
        if any(word in normalized_target for word in ("search", "query", "input", "text")):
            for selector in (
